...
/* ===== HU: Zonas especiales ===== */
const ZONAS_ESPECIALES = ["Ninguna", "Panamá Pacífico", "Zona Franca", "SEM"] as const;
...
export const Priorizacion: React.FC = () => {
  ...
  const renderActividadChips = (selected: any) => {
    ...
    // ✅ solo mostramos el primero como chip en el select (para que sea consistente)
    const first = arr[0];
  ...
  return (
    <Box>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField select fullWidth label="Provincia" name="provincia" value={form.provincia} onChange={handleChange}>
            {PROVINCIAS.map((p) => (
              <MenuItem key={p} value={p}>
                {p}
              </MenuItem>
